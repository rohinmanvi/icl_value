...
class GeneratedTrajectory:
    ...
    token_candidates: List[List[Tuple[int, float, float]]]  # [(token_id, ref_logprob, q_value), ...]
...
def _generate_with_q_weighting(
    ...
    token_candidates: List[List[Tuple[int, float, float]]] = []
    ...
    for step in range(cfg.max_new_tokens):
        ...
        # Store candidates (sorted by Q-value)
        candidates = [(cand_indices[i], cand_ref_lps[i], cand_q_values[i]) for i in range(num_cands)]
...
def _evaluate_trajectory_q_values(
    ...
    Returns:
        ...
        token_candidates: [(token_id, ref_logprob, q_value), ...] for each position
    """
    ...
    for i, tid in enumerate(response_ids):
        ...
        ref_log_probs = F.log_softmax(pos_logits.float(), dim=-1)
        ref_probs = ref_log_probs.exp()
        ...
        # For candidates, we'd need additional forward passes (expensive)
        # For visualization of existing trajectories, just show actual token
        cand_lps = ref_log_probs[cand_indices].tolist()

        # Approximate: use actual Q for actual token, don't compute for others
        candidates = []
        for j, cid in enumerate(cand_indices):
            if cid == tid:
                candidates.append((cid, cand_lps[j], actual_q))
            else:
                # Use ref logprob as proxy (not accurate but fast)
                candidates.append((cid, cand_lps[j], 0.5))  # Neutral Q
    ...
    return token_q_values, token_candidates
    ...
    for local_i, pid in enumerate(my_prompt_ids):
        ...
        if num_ref_to_show > 0:
            ...
            for ref_i in range(num_ref_to_show):
                ...
                # Evaluate Q-values
                q_values, candidates = _evaluate_trajectory_q_values(
                    ref_model=ref_model,
                    critic_model=critic_model,
                    tokenizer=tokenizer,
                    rollout=rollout,
