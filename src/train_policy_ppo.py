#!/usr/bin/env python3
"""
Offline PPO policy training from pre-generated rollouts.
...
Key idea:
  ...
  - Compute log π_new(a_t | s_t) via a forward pass of the trainable model.
  - Define a per-token value from critic predictions:
        V_t = log((p_t + tau) / (1 + tau))
    where p_t is `critic_p_correct[t]` aligned to the t-th output token.
  - Compute per-token advantages using GAE (gamma=1):
        A_0 = 0
        delta_t = V_t - V_{t-1}               (for t >= 1)
        A_t = delta_t + lambda * A_{t+1}      (computed backwards; for t >= 1)
  - Optimize the standard PPO clipped objective on these fixed actions.

Optional KL regularization:
  - Since only reference action logprobs are available (not full ref logits),
    we add a simple, always-nonnegative reverse-KL estimate:
        kl_rev = exp(d) - 1 - d,  where d = logπ_new - logπ_ref
    whose expectation under the reference policy equals KL(ref || new).
"""
...
def parse_args() -> Config:
    ...
    p.add_argument("--output-dir", default="models/policy_ppo")
    ...
    p.add_argument("--adv-clip", type=float, default=0.0, help="If >0, clamp advantages to [-adv-clip, adv-clip].")
...
def _compute_advantages(
    ...
    We treat critic_p_correct[t] as a per-token estimate of P(success | prefix up to token t),
    define a log-prob "value" V_t, and compute deltas purely from value differences (gamma=1).
    The first token advantage is set to 0 by convention.
    """
    values: List[float] = []
    for p in p_correct:
        try:
        ...
        # Smoothed log-probability; stays <= 0 for p in [0,1] and tau > 0.
    ...
    # For t>=1: delta_t = V_t - V_{t-1}; A_t = delta_t + lam * A_{t+1}.
...
def _train_worker(local_rank: int, world_size: int, cfg: Config) -> None:
    ...
    if master and cfg.wandb_project:
        try:
            ...
            run_name = cfg.wandb_name or f"ppo_{int(time.time())}"
            wb = wandb
            wb.init(
