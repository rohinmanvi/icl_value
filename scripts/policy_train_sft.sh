...
#SBATCH --job-name=icl_policy_sft
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gpus-per-node=8
#SBATCH --cpus-per-gpu=12
#SBATCH --output=/home/rohin/icl_value/logs/policy_sft.out
#SBATCH --error=/home/rohin/icl_value/logs/policy_sft.err
...
output_dir="models/policy_sft_qwen17b_correct"

dp_size=8
micro_batch_size=1
grad_accum_steps=1
learning_rate=2e-5
samples_per_prompt=8  # set -1 to use all correct rollouts
correct_threshold=0.5
max_steps=-1

echo "Starting SFT policy training (correct rollouts only):"
echo "  Model:            $model_id"
echo "  Data:             $data_path"
...
echo "  Correct threshold:$correct_threshold"
echo "  Max steps:        $max_steps"
echo "  Start time:       $(date)"

python3 -u src/train_policy_sft.py \
  --model-id "$model_id" \
  --data-path "$data_path" \
  ...
  --gradient-checkpointing 2>&1 | tee -a /home/rohin/icl_value/logs/policy_sft_$(basename ${output_dir}).log
